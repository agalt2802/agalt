...
let key = fs.readFileSync("C:\\Users\\agalt\\OneDrive\\Desktop\\Socket\\server\\Cert\\server.key");
let cert = fs.readFileSync("C:\\Users\\agalt\\OneDrive\\Desktop\\Socket\\server\\Cert\\server.crt");
let options = {
  key: key,
  cert: cert
};
...
app.get("/batteryInfo", function (req, res) {
  console.log(batteryStatus);
  res.render("batteryInfo", {
    batteryStatus: batteryStatus.level,
    IsPlugged: batteryStatus.isPlugged
  });
});
...
app.get("/recieveMessage", function (req, res) {
  ...
  res.render("recieveMessage", {
    messagesRecieved: JSON.stringify(messagesRecieved)
  });
});
